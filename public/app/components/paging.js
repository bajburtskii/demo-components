(function () {
	...
	// .controller('PagingFilterCtrl', PagingFilterCtrl)
	;
	...
	function paging(HttpService, $location, $state, $stateParams, $timeout, $rootScope) {
		...
		function internalAction(scope, page) {
			...
			HttpService.getWParams(scope.url, {params: $location.search()}, function (data) {
				...
				scope.page = data.current_page || 1;
				scope.pageSize = data.per_page || 12;
				scope.total = data.total || 0;

				// If group by
				if(scope.group && scope.group.length) {
					scope.collection = scope.group && scope.group.length ? _.groupBy(data.data, function (x) {
						return x[scope.group];
					}) : data.data;
				} else if(scope.groupFn && typeof scope.groupFn == 'function') {
					scope.collection = scope.groupFn()(data.data);
				} else {
		...
		return {
			...
			'<span class="page-link" ng-bind="Item.value"></span> ' +
			'</li>' +
			'</ul>',
			...
			link: function (scope, element, attrs) {
				...
				var stopWatch = $rootScope.$watchCollection('filter', function (newValue, oldValue) {
					...
					if (newValue !== oldValue) {
						...
						if (newValue) delete newValue.page;
						if (oldValue) delete oldValue.page;
